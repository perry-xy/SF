...
class FacilityLocation(object):
    ...
    def facility_location(self):
            ...
            for rdc_name in rdc_cand:
                model.addConstr((sum((f_c[rdc_name, name] * demand[name]['demand_sum']
                                ...
                                sum((f_c[rdc_name, name] * demand[name]['demand_sum'] for name in
                                     customer)) * rr_c * self.config.return_turnover_days
        ...
        i_rdc_C, q_rdc_C, w_rdc_C, i_rdc_c, q_rdc_c, w_rdc_c, q_rdc_sum, i_rdc_c_r = self.rdc_c_temp_calc(f_c)
        i_rdc_B, q_rdc_B, w_rdc_B, i_rdc_b, q_rdc_b, w_rdc_b, i_rdc_b_r = self.rdc_b_temp_calc(f_b)
        ...
        shipping_cost_b, shipping_cost_c = self.rdc_shipping_cost(q_rdc_c, w_rdc_c, q_rdc_b, f_b)
        rdc_storage, rdc_storage_c, rdc_storage_b, rdc_storage_cost_d = self.rdc_storage_cost(i_rdc_c, i_rdc_b)
        rdc_inbound, rdc_inbound_c, rdc_inbound_c, rdc_outbound, rdc_outbound_c, rdc_outbound_b, \
        rdc_inbound_cost, rdc_outbound_cost = self.rdc_handling_cost(q_rdc_C, q_rdc_B)
    ...
    def rdc_shipping_cost(self, d_rdc_c, w_rdc_C, q_rdc_B, f_b):
        """
        ...
        distribution_price = self.data_class.distribution_price
        upc = self.config.upc
        carton_volumn = self.config.carton_volumn
        # declare variables
        shipping_cost_d = {}
        shipping_cost_d_b = {}
        shipping_cost_d_c = {}
        ...
        for rdc_name in rdc_cand:
            ...
            ship_toB = 0
            for name in customer_2b:
                v_rdc_tmp = q_rdc_B[rdc_name, name]/demand_2b[name]['replenishment']/upc * carton_volumn
                ship_toB += v_rdc_tmp * distribution_price[rdc_name, name]['weight_price_qty_toB']
    ...
    def rdc_storage_cost(self, i_rdc_c, i_rdc_b):
        ...
        for rdc_name in rdc_cand:
            ...
            rdc_storage_b_sum = 0
            for name in customer:
                rdc_storage_tmp = i_rdc_c[rdc_name, name]/rdc_cost_loc[rdc_name]['unit_area_toC'] \
            ...
            for name in customer_2b:
                rdc_storage_tmp_b = i_rdc_b[rdc_name, name]/rdc_cost_loc[rdc_name]['unit_area_toB'] \
                                  * 12 * rdc_cost_loc[rdc_name]['monthly_rental_price']
        ...
        return rdc_storage, rdc_storage_c, rdc_storage_b, rdc_storage_cost

    def reverse_cost(self, i_rdc_c_r, i_rdc_b_r):
        """
        ...
        rdc_storage_b = 0
        for rdc_name in rdc_cand:
            rdc_storage_c += i_rdc_c_r[rdc_name] / rdc_cost_loc[rdc_name]['unit_area_toC'] \
                * 12 * rdc_cost_loc[rdc_name]['monthly_rental_price']
            rdc_storage_b += i_rdc_b_r[rdc_name] / rdc_cost_loc[rdc_name]['unit_area_toB'] \
                * 12 * rdc_cost_loc[rdc_name]['monthly_rental_price']

        reverse_shipping_cost = 0
        reverse_inbound_cost = 0
        reverse_outbound_cost = 0
        reverse_storage_cost = rdc_storage_c + rdc_storage_b
        return reverse_shipping_cost, reverse_inbound_cost, reverse_outbound_cost, reverse_storage_cost

    def rdc_handling_cost(self, q_rdc_c, q_rdc_b):
        """
        ...
        for rdc_name in rdc_cand:
            # toC handling
            rdc_inbound_c_tmp = (q_rdc_c[rdc_name]['sum'])/upc * rdc_cost_loc[rdc_name]['inbound_carton_price_toC']

            rdc_outbound_c_tmp = (q_rdc_c[rdc_name]['sum'])/upo * rdc_cost_loc[rdc_name]['outbound_order_price_toC']
            rdc_inbound_c += rdc_inbound_c_tmp
            rdc_outbound_c += rdc_outbound_c_tmp
            # toB handling
            rdc_inbound_b_tmp = (q_rdc_b[rdc_name]) /upc * rdc_cost_loc[rdc_name]['inbound_carton_price_toB']

            rdc_outbound_b_tmp = (q_rdc_b[rdc_name]) /upc * rdc_cost_loc[rdc_name]['outbound_carton_price_toB']

            rdc_inbound_b += rdc_inbound_b_tmp
            rdc_outbound_b += rdc_outbound_b_tmp
            # rdc_r_outbound += rdc_outbound_tmp * self.config.rr_cdc
            rdc_inbound = rdc_inbound_b + rdc_inbound_c
            rdc_outbound = rdc_outbound_c + rdc_outbound_b
            rdc_inbound_cost[rdc_name] = rdc_inbound_c_tmp + rdc_inbound_b_tmp
            rdc_outbound_cost[rdc_name] = rdc_outbound_c_tmp + rdc_outbound_b_tmp

        return rdc_inbound, rdc_inbound_c, rdc_inbound_c, rdc_outbound, rdc_outbound_c, rdc_outbound_b,\
               rdc_inbound_cost, rdc_outbound_cost
    ...
    def rdc_c_temp_calc(self, f_c):
        ...
        return_turnover_days = self.config.return_turnover_days
        rr_c = self.config.rr_c
        total_days = 365
        # declare variables
        q_rdc = {}
        ...
        for rdc_name in rdc_cand:
            ...
            for name in customer:
                ...
                # reverse
                i_rdc_tmp_r = rr_c*(f_c[rdc_name, name] * demand[name]['demand_sum']) / total_days * return_turnover_days
    ...
    def rdc_b_temp_calc(self, f_b):
        ...
        for rdc_name in rdc_cand:
            ...
            for name in customer:
                ...
                i_rdc_tmp_r += rr_b * (f_b[rdc_name, name] * demand[name]['demand_sum']) / total_days * return_turnover_days
            ...
            i_rdc_r[rdc_name] = i_rdc_tmp_r
        return i_rdc, q_rdc, w_rdc, i_rdc_b, q_rdc_b, w_rdc_b, i_rdc_r
    ...
    def rdc_post_process(self):
        ...
        rdc_storage, rdc_storage_c, rdc_storage_b, rdc_storage_cost = self.rdc_storage_cost(i_rdc_c, i_rdc_b)
        rdc_inbound, rdc_inbound_c, rdc_inbound_c, rdc_outbound, rdc_outbound_c, rdc_outbound_b, \
        rdc_inbound_cost, rdc_outbound_cost = self.rdc_handling_cost(q_rdc_C, q_rdc_B)
        # rdc_capital_cost, rdc_capital_cost_d = self.capital_cost(i_rdc_c)
        ...
        for rdc_name in rdc_open_valid:
            ...
            rdc_output['storage_cost'] = rdc_storage_cost[rdc_name]
            ...
            rdc_output['Inventory_C'] = i_rdc_C[rdc_name]
            rdc_output['Inventory_B'] = i_rdc_B[rdc_name]
            rdc_output['Area_C'] = i_rdc_C[rdc_name] / rdc_cost_loc[rdc_name]['unit_area_toC']
            rdc_output['Area_B'] = i_rdc_B[rdc_name]/rdc_cost_loc[rdc_name]['unit_area_toB']
        ...
        return df, rdc_shipping_cost, shipping_cost_b, shipping_cost_c, \
               rdc_storage, rdc_inbound, rdc_outbound, rdc_r_outbound, rdc_capital_cost
