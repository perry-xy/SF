...
class FacilityLocation(object):
    ...
    def __init__(self, data, config):
        ...
        self.model = None
        self.rdc_open = None
        self.rdc_customer_category = None
    ...
    def facility_location(self):
        ...
        rdc_customer_category = model.addVars(rdc_cand, customer, category_list,
                                              vtype=GRB.CONTINUOUS, name='rdc_customer_category')
        ...
        #     #            '772','791', '851', '871', '991']
        # rdc_use = ['28', '311', '760', '571', '531', '711']
        ...
        for name in customer:
            for category in category_list:
                model.addConstr(rdc_customer_category.sum('*', name, category) == demand[name][category],
                                name="customer_category_demand_constr")

        for rdc_name in rdc_cand:
            for category in category_list:
                model.addConstr(rdc_customer_category.sum(rdc_name, "*", category)
                                == cdc_rdc_category.sum("*", rdc_name, category),
                                name="rdc_category_demand_constr")
...
#       # TODO: modify the capacity constraint
        for rdc_name in rdc_cand:
            # area_sum = 0
            for name in customer:
                # area_customer = 0
                for category in category_list:
                #     area_customer = (rdc_customer_category[rdc_name, name, category]/YEAR_DAY
                #                 * category_info[category]['turn_over_day']
                #                  + category_info[category]['safety_inventory']) * category_info[category]['area']
                # area_sum += area_customer
                    model.addConstr(rdc_customer_category[rdc_name, name, category]
                            <= rdc_open[rdc_name] * rdc_capacity[rdc_name], name='rdc_capacity')
        ...
        rdc_shipping_cost, rdc_shipping_cost_d = self.rdc_shipping_cost(rdc_customer_category)
        rdc_storage_cost, rdc_storage_cost_d = self.rdc_storage_cost(rdc_customer_category)
        rdc_inbound, rdc_outbound, rdc_inbound_cost, rdc_outbound_cost = self.rdc_handling_cost(rdc_customer_category)
        ...
        cost = cdc_shipping_cost + rdc_shipping_cost + rdc_storage_cost + rdc_inbound + rdc_outbound
        ...
        if model.Status == GRB.OPTIMAL:
            ...
            self.rdc_customer_category = model.getAttr('x', rdc_customer_category)
    ...
    def cdc_shipping_cost(self, cdc_rdc_category):
        ...
        for cdc_name in cdc_rand:
            ...
            for rdc_name in rdc_cand:
                for category in category_list:
                    ship_cdc_rdc += cdc_rdc_category[cdc_name, rdc_name, category] * category_info[category]['weight'] \
    ...
    def rdc_shipping_cost(self, rdc_customer_category):
        """
        ...
        for rdc_name in rdc_cand:
            ...
            for name in customer:
                demand_weight = 0
                for category in category_list:
                    demand_weight += rdc_customer_category[rdc_name, name, category]*category_info[category]['weight']
                    ship_c += demand_weight/self.config.weight_avg * \
    ...
    def rdc_storage_cost(self, rdc_customer_category):
        """
        ...
        for rdc_name in rdc_cand:
            ...
            for name in customer:
                customer_area = 0
                for category in category_list:
                    customer_area += (rdc_customer_category[rdc_name, name, category] /
                           YEAR_DAY * category_info[category]['turn_over_day']/2 +
                                      category_info[category]['safety_inventory']) * category_info[category]['area']
                rdc_storage_c_sum += customer_area * 12 * rdc_cost_loc[rdc_name]['monthly_rental_price']
    ...
    def rdc_handling_cost(self, rdc_customer_category):
        """
        ...
        for rdc_name in rdc_cand:
            ...
            for name in customer:
                customer_weight = 0
                for category in category_list:
                    customer_weight += rdc_customer_category[rdc_name, name, category]*category_info[category]['weight']
    ...
    def rdc_post_process(self):
        ...
        category_list = self.config.category_list
        category_info = self.data_class.category_info
        rdc_customer_category = self.rdc_customer_category
        ...
        for rdc_name in rdc_open_valid:
            ...
            rdc_output['Inventory'] = sum(rdc_customer_category[rdc_name, name, category]/YEAR_DAY
                                          * category_info[category]['turn_over_day']/2
                                          + category_info[category]['safety_inventory']
                                          for name in customer for category in category_list)
            rdc_output['Area'] = sum((rdc_customer_category[rdc_name, name, category]/YEAR_DAY
                                          * category_info[category]['turn_over_day']/2
                                          + category_info[category]['safety_inventory']) * category_info[category]['area']
                                          for name in customer for category in category_list)
            # rdc_output['Capital_Cost'] = rdc_capital_cost_d[rdc_name]
            rdc_output['Quantity'] = sum(rdc_customer_category[rdc_name, name, category]
                                         for name in customer for category in category_list)
            rdc_output['Quantity_SKU1'] = sum(rdc_customer_category[rdc_name, name, "SKU1"]
                                         for name in customer )
            rdc_output['Quantity_SKU2'] = sum(rdc_customer_category[rdc_name, name, "SKU2"]
                                         for name in customer )
            rdc_output['Quantity_SKU3'] = sum(rdc_customer_category[rdc_name, name, "SKU3"]
                                         for name in customer )
            rdc_output['Quantity_SKU4'] = sum(rdc_customer_category[rdc_name, name, "SKU4"]
                                         for name in customer )
            rdc_output['Quantity_SKU5'] = sum(rdc_customer_category[rdc_name, name, "SKU5"]
                                         for name in customer )
            rdc_output['Quantity_SKU6'] = sum(rdc_customer_category[rdc_name, name, "SKU6"]
                                         for name in customer)
    ...
    def c_end_network(self):
        ...
        for rdc_name in rdc_open_valid.keys():
            ...
            for c_name in customer:
                    ...
                    c_network['QUANTITY'] = sum(rdc_customer_category[rdc_name, c_name, category]
                                                for category in category_list)
                    c_network['WEIGHT'] = sum(rdc_customer_category[rdc_name, c_name, category]
                                              * category_info[category]['weight'] for category in category_list)
            ...
            for rdc_name in rdc_open_valid.keys():
                ...
                for category in category_list:
                    if cdc_rdc_category[cdc_name, rdc_name, category] >= 1:
                        c_network[category] = cdc_rdc_category[cdc_name, rdc_name, category]
                        # c_network['WEIGHT'] = sum(rdc_customer_category[rdc_name, c_name, category]
                        #                           * category_info[category]['weight'] for category in category_list)
                        # c_network['SLA'] = f_c[rdc_name, c_name] * distribution_price[rdc_name, c_name]['sla_toC']
                        # c_network['TIME'] = distribution_price[rdc_name, c_name]['time_median_toC']
