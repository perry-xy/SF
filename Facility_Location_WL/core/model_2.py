...
class FacilityLocation(object):
    ...
    def facility_location(self):
        ...
        demand_rdc = model.addVars(cdc_cand, rdc_cand, vtype=GRB.CONTINUOUS, name='demand_rdc')
        ...
        for rdc_name in rdc_cand:
            ...
            for name in customer:
                customer_demand += f_c[rdc_name, name] * demand[name]['demand_weight_sum']
            model.addConstr(demand_rdc.sum('*', rdc_name) - customer_demand == 0, name='demand_cons')
        ...
        cdc_shipping_cost, cdc_shipping_cost_d = self.cdc_shipping_cost(demand_rdc)
        rdc_shipping_cost, rdc_shipping_cost_d = self.rdc_shipping_cost(f_c)
    ...
    def cdc_shipping_cost(self, rdc_demand):
        """
        ...
        for cdc_name in cdc_rand:
            ship_cdc_rdc = 0
            for rdc_name in rdc_cand:
                ship_cdc_rdc += rdc_demand[cdc_name, rdc_name] \
    ...
    def cdc_post_process(self):
        ...
        for cdc_name in cdc_cand:
            ...
            cdc_output['Weight'] = sum(demand_rdc[cdc_name, rdc_name]
                                         for rdc_name in rdc_cand)
