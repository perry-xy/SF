...
class Optimization(object):
    ...
    def algorithm(self,day,strategy):
        ...
        while gen<maxGen:
            ...
            zone_selected = random.sample(set(zone).difference(set(zone_taboo)), 1)[0]  # 选一个zone
            em_selected_add = random.sample(set(em_idle).difference(set(em_taboo)), 1)[0] # 未被选中的小哥里选一个
    ...
    def algorithm1(self,day,strategy):
        zone = list(strategy.keys())   # key：zone
        em_ori = self.data.employees
    ...
    def optimize(self):
        ...
        for day, strategy in solution.items():    # day:日期；strategy:排班结果
