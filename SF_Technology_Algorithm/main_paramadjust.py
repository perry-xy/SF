...
class Model_Select_Config():
    ...
    """
    predict_target = 'pai_num'     # pai_num / shou_num
    ...
    if use_all_feature:
        Config.target_column = ['pai_num', 'shou_num']
    else:
...
if Model_Select_Config.model_type == 'all': # 如果总体训练
    ...
    if Model_Select_Config.params_optimization == False: # 总体训练 & 不进行参数优化
        ...
        test_mape, test_mape_result, test_results, feature_importance_df, model, train_results \
                                    = model_evaluate(handled_data, params = Config.default_params, test_rounds = 7  ,
                                                                                                    test_period = 3)
        ...
        pd.Series(test_mape_result, index = index, name = Config.metric[0]).to_csv('{}/{}_Mape_test_result_noopti.csv'
            .format(output_path, Model_Select_Config.predict_target))                        # 7轮迭代的mape
        train_results.to_csv('{}/{}_train_results_noopti.csv'
            .format(output_path, Model_Select_Config.predict_target),index=False)            # 最后一轮迭代的整体预测结果
        test_results.to_csv('{}/{}_test_results_noopti.csv'
            .format(output_path, Model_Select_Config.predict_target), index = False)         # 7轮迭代的预测值
        feature_importance_df.to_csv('{}/{}_feature_importance_noopti.csv'
            .format(output_path, Model_Select_Config.predict_target), index = True)          # 7轮迭代的feature_importance
        ...
        # 测试集上迭代7次
        test_mape, test_mape_result, test_results, feature_importance_df, model, train_results = \
                                                    model_evaluate(handled_data, params=best_params, test_rounds=7, test_period=3)
        print("21天交叉验证后的泛化误差为：{}".format(str(test_mape)))
        ...
        pd.Series(test_mape_result, index=index, name=Config.metric[0]).to_csv('{}/{}_Mape_test_result_opti.csv'
            .format(output_path, Model_Select_Config.predict_target))                      # 7轮迭代的mape
        train_results.to_csv('{}/{}_train_results_opti.csv'
            .format(output_path, Model_Select_Config.predict_target), index=False)         # 最后一轮迭代的整体预测结果
        test_results.to_csv('{}/{}_test_results_opti.csv'
            .format(output_path, Model_Select_Config.predict_target), index=False)         # 7轮迭代的预测值
        feature_importance_df.to_csv('{}/{}_feature_importance_opti.csv'
            .format(output_path, Model_Select_Config.predict_target), index=True)          # 7轮迭代的feature_importance
        # bayes_process_df.to_csv('{}/{}_bayes_process_opti.csv'
        #     .format(output_path, Model_Select_Config.predict_target), index=False)             # 参数优化的过程记录
