...
class DataHandler(object):
    ...
    def _load_rawdata(self):
        ...
        try:
            ...
            #                                                               dtype=object).value
            self._demands = pd.read_excel(self.PATH, sheet_name = '预测信息')
            ...
            #                                                               dtype=object).value
            self._predict_samples = pd.read_excel(self.PATH, sheet_name = '样本区间')
            # wb.close()
            # app.quit()
...
if __name__ == "__main__":
    ...
    app = xw.App(visible=False, add_book=False)
    data = pd.read_excel('data/1130_data_input_level.xlsx')
    print(data.head())
    wb = app.books.open('data/1130_data_input_level.xlsx')
    employees = wb.sheets["小哥信息"].range('A1').options(pd.DataFrame, expand='table', index=False,
                                                            dtype=object).value
