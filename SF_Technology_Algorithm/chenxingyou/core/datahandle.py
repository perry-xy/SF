...
class DataHandler():
    ...
    def data_transform(self):
        ...
        # outside_data = self.outside_data()  #
        # df = pd.merge(df, outside_data, on='date', how = 'left') #
    ...
    def outside_data(self, filepath='data/sf_all_data.csv'):
        """
        读入大网的件量数据
        :param filepath:
        :return:
        """
        outside_data = pd.read_csv(filepath)
        dt_format = '%Y%m%d'
        outside_data['date'] = outside_data['date'].astype(str)
        outside_data['date'] = pd.to_datetime(outside_data['date'], format=dt_format)
