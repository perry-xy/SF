...
class FeatureEngineer():
    ...
    def feature_data_transform(self):
        ...
        sales_date = self.sales_feature(date_from = '20200618', date_end = '20200622')
        ...
        # feature_df = pd.concat([feature_df, group_feature_new], axis = 1)
        # print(feature_df.shape)
        # feature_df.to_csv('group_feature_new.csv')
    ...
    def group_feature_new(self, time_handled_df):
        """
        ...
        for target_column in self.target_column:
            df_zone = pd.DataFrame()
            for zone_id, groups in time_handled_df.groupby(self.id_column):
                df_zone_shift = pd.DataFrame()  # 每一个zone的滑窗结果
                zone_value = groups[[target_column]]  # DataFrame，index保持不变
                zone_columns = list()
                for  shift_num in [7,14,21,28]:
                    col_name = '{}_shift_{}'.format(target_column, shift_num)
                    zone_columns.append(col_name)
                    cur_series = zone_value.shift(shift_num) # 滑窗，index不会跟着移动
                    cur_series.rename({ target_column: col_name}, axis=1, inplace=True)
            ...
            df_zone[group_column[4]] = df_zone.max(axis = 1), df_zone.min(axis = 1), df_zone.median(axis = 1), df_zone.std(axis = 1),\
                                        df_zone.mean(axis=1)
            df_zone.drop(['{}_shift_{}'.format(target_column,shift_num) for shift_num in [7,14,21,28]],axis=1,inplace=True)
...
if __name__ == '__main__':
    ...
    model_data = model_data_split(handled_data, Config.id_column, Config.time_column, predict_period=21)
