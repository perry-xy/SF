...
class ResultFormat():
    ...
    def handle_cdc_work(self, cdc_category_capacity_df):
        ...
        column_name =  ['供应商代码','供应商城市','供应商_LAT','供应商_LGT','RDC代码','RDC城市','RDC_LAT','RDC_LGT','SKU_NO','品类','重量(kg)']
        ...
        for idx, row in cdc_category_capacity_df.iterrows():
            ...
            for sku in self.model_result.data_input.category_list:
                if row[sku.upper()] > 0:
                    ...
                    df = pd.DataFrame([cdc,cdc_cn,cdc_lat,cdc_lgt,rdc,rdc_cn,rdc_lat,rdc_lgt,sku.upper(),sku_cn,sku_weight]).T
                    df.columns = column_name
                    cdc_category_capacity = cdc_category_capacity.append(df)

        cdc_category_capacity.sort_values(by = ['RDC代码','SKU_NO'], ascending = [True,True], ignore_index = True, inplace = True)
