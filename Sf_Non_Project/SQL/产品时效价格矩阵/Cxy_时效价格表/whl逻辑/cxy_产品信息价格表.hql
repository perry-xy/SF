...
drop table tmp_dm_as.cxy_joanna_price_base_1;
create table tmp_dm_as.cxy_joanna_price_base_1 as
select 
	a.product_name,
	a.product_code,
	flow_type,
...
from 
	...
	where inc_day =  '20200830'   --'$[day(yyyyMMdd,-1)]'
		and (invalid_date is null or substr(invalid_date,1,10) > '2020-08-30'))t   --'$[day(yyyy-MM-dd,-1)]' ) ) t 
	where t.rn = 1 
	) a --中文名变成产品码
left join 
	(select * from 
		...
		where inc_month = '20200830'--'$[day(yyyyMMdd,-1)]'
			and (invalid_date is null or substr(invalid_date,1,10) > '2020-08-30')--'$[day(yyyy-MM-dd,-1)]' )
...
drop table tmp_dm_as.cxy_joanna_price_base_2;
create table tmp_dm_as.cxy_joanna_price_base_2 as
select 
	product_name,
	product_code,
	flow_type,
	...
	price_code,
	data_type
from tmp_dm_as.cxy_joanna_price_base_1
where data_type = 'new'
union all
select 
	a.product_name,
...
left join 
	(select * from 
		...
		where inc_month ='202007' --'$[month(yyyyMM,-1)]'
			and (invalid_date is null or substr(invalid_date,1,10) > '2020-08-30')--'$[day(yyyy-MM-dd,-1)]' )
...
drop table tmp_dm_as.cxy_joanna_price_attach_name;
create table tmp_dm_as.cxy_joanna_price_attach_name as
select
	dist_code,
...
-- 流向名称匹配
drop table tmp_dm_as.cxy_joanna_price_base_3;
create table tmp_dm_as.cxy_joanna_price_base_3 as
select 
	product_name,
	product_code,
	flow_type,
	...
	price_code
from 
	tmp_dm_as.cxy_joanna_price_base_2 a
left join 
	tmp_dm_as.cxy_joanna_price_attach_name c 
on a.source_code = c.dist_code
left join 
	tmp_dm_as.cxy_joanna_price_attach_name d 
on a.dest_code = d.dist_code;

drop table tmp_dm_as.cxy_joanna_price_base_4_0;
create table tmp_dm_as.cxy_joanna_price_base_4_0 as
select * 
from 
	(select 
...
--SE0117	顺丰特惠C
--SE0118	顺丰特惠B
drop table tmp_dm_as.cxy_joanna_price_base_4;
create table tmp_dm_as.cxy_joanna_price_base_4 as
select 
	a.*,
	c.base_weight_qty,
	c.base_price,
...
from 
	(select * 
	from tmp_dm_as.cxy_joanna_price_base_3
	where product_code not in ('SE0030','SE0117','SE0118','T6-L')      --'T6-L'特惠D暂不考虑 
	) a
left join tmp_dm_as.cxy_joanna_price_base_4_0 b --这里也有主键不唯一的数据要处理一下
		...
		(select 
			...
			from 
				...
				from 
					...
					where inc_month ='20200830') aa --'$[day(yyyyMMdd,-1)]'   ) aa     --优先取-1
				) ss where rd = 1
		) c1
	where rn = 1
	) c --这里有不唯一的数据，开发去维护了
...
-- 特惠B对应1  SE0118	顺丰特惠B
drop table tmp_dm_as.cxy_joanna_price_base_5;
create table tmp_dm_as.cxy_joanna_price_base_5 as
select 
	a.*,
	c.base_weight_qty,
	c.base_price,
...
--C对应2  --SE0117	顺丰特惠C  
drop table tmp_dm_as.cxy_joanna_price_base_6;
create table tmp_dm_as.cxy_joanna_price_base_6 as
select 
	a.*,
	c.base_weight_qty,
	c.base_price,
...
drop table tmp_dm_as.cxy_joanna_price_base_7;
create table tmp_dm_as.cxy_joanna_price_base_7 as
select 
	a.*,
	c.base_weight_qty,
	c.base_price,
		...
		(select 
			...
			from 
				...
				from 
					...
					where inc_day = '20200830') aa--'$[day(yyyyMMdd,-1)]'   ) aa   
				) ss where rd = 1
		) c1
	where rn = 1
	) c --这里有不唯一的数据，开发去维护了
...
-- 汇总
drop table tmp_dm_as.cxy_joanna_price_base_all;
create table tmp_dm_as.cxy_joanna_price_base_all as
select 
	product_name,
	product_code,
	flow_type,
	...
	max_weight_qty,
	light_factor
from tmp_dm_as.cxy_joanna_price_base_4
union all
select
	product_name,
	product_code,
	...
	max_weight_qty,
	light_factor
from tmp_dm_as.cxy_joanna_price_base_5
union all
select
	product_name,
	product_code,
	...
	max_weight_qty,
	light_factor
from tmp_dm_as.cxy_joanna_price_base_6
union all
select
	product_name,
	product_code,
	...
	max_weight_qty,
	light_factor
from tmp_dm_as.cxy_joanna_price_base_7;
...
insert overwrite table dm_as.cxy_joanna_price_base_all partition (inc_day = '$[day(yyyyMMdd,-1)]') 
select
	product_name,
	product_code,
	flow_type,
	...
	max_weight_qty,
	light_factor
from tmp_dm_as.cxy_joanna_price_base_all;
