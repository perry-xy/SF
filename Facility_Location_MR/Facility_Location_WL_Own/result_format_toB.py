...
class ResultFormat():
    ...
    def rdc_post_process(self):
        ...
        shipping_cost, shipping_cost_trunk, shipping_cost_transit, shipping_cost_dist, shipping_cost_d, \
        shipping_distance, shipping_diistance_d = self.model_result.rdc_shipping_cost(r_c)
        rdc_storage_cost, rdc_storage_cost_d = self.model_result.rdc_storage_cost(r_c)
        rdc_inbound, rdc_outbound, rdc_inbound_d, rdc_outbound_d = self.model_result.rdc_handling_cost(r_c)
        reverse_cost, reverse_distance, reverse_cost_d, reverse_distance_d = self.model_result.reverse_cost(r_c)

        m = 0
        for rdc_name in rdc_open_valid:
            rdc_output['RDC'] = rdc_name
            rdc_output['City_name'] = city_add[rdc_name]['city_name_cn']
            rdc_output['Trunk_Cost'] = shipping_cost_d[rdc_name]['trunk']
            rdc_output['Transit_Cost'] = shipping_cost_d[rdc_name]['transit']
            rdc_output['Dist_Cost'] = shipping_cost_d[rdc_name]['Dist']
            rdc_output['Shipping_Cost'] = shipping_cost_d[rdc_name]['Cost']
            ...
            rdc_output['Area'] = sum((r_c[rdc_name, name] * demand[name][category] / self.model_result.config.YEAR_DAY
            ...
            rdc_output['Customer']=sum(r_c[rdc_name, name] for name in customer)
    ...
    def performance(self,m):
        ...
        rdc_storage_cost, rdc_storage_cost_d = self.model_result.rdc_storage_cost(self.model_result.r_c)
        rdc_inbound, rdc_outbound, rdc_inbound_d, rdc_outbound_d = self.model_result.rdc_handling_cost(self.model_result.r_c)
        reverse_cost, reverse_distance, reverse_cost_d, reverse_distance_d = self.model_result.reverse_cost(self.model_result.r_c)
        # performance
        ...
        performance_output['Dist_Shipping_cost'] = shipping_cost
        performance_output['Trunk_cost'] = cdc_shipping_cost
