...
class FacilityLocation():
    ...
    def facility_location(self):
        ...
        # 干线运输价格、支线运输价格、sku类别、两种需求点数据、指定rdc仓数
        ...
        shipping_distance, shipping_diistance_d = self.rdc_shipping_cost(r_c)
        ...
        cost = cdc_shipping_cost + shipping_cost + rdc_storage_cost + rdc_inbound + rdc_outbound + reverse_cost
    ...
    def rdc_shipping_cost(self, r_c):
        ...
        for rdc_name in rdc_cand:
            ...
            for name in customer:
                ...
                dist += r_c[rdc_name, name] * demand[name]['weight'] * dist_price[rdc_name]['dist_fee']
            ...
            for category in category_list:
                ...
                for r_name in reverse_customer:
                    area += r_r_c[rdc_name,r_name] * reverse_demand[r_name][category] / \
                                     self.config.YEAR_DAY * sku[category]['turnoverdays'] * sku[category]['area_weight_ratio']
            rdc_area += area * self.config.area_ratio
            rdc_area_d[rdc_name] = area * self.config.area_ratio
            rdc_storage_cost += self.config.area_ratio * area * 12 * warehouse[rdc_name]['rental']
            rdc_storage_cost_d[rdc_name] = self.config.area_ratio * area * 12 * warehouse[rdc_name]['rental']
